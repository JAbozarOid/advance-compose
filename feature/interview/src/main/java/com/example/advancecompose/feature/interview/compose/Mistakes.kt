...
fun Screen() {
    ...
    /**
     * If you set shouldNavigate = true directly inside the composable (outside LaunchedEffect),
     * it would reassign every recomposition → infinite recompositions or unexpected behavior.
     */
 ...
 * # 2
 * reading state early
 * This code explains an important Jetpack Compose concept called "reading state early vs. reading state late"
 */
@Composable
fun TransactionHistory() {
    Box(modifier = Modifier.fillMaxSize()) {
        val scrollState = rememberScrollState(0)
        // Every time scrollState.value changes, TransactionHistory and TitleBar will both recompose
...
fun TitleBar(title: String, scroll: Int) {
    val offset = with(LocalDensity.current) { scroll.toDp() } //  Reading State Late

    // y = offset -> call this lambda only when it needs the value
...
/**
 * In Jetpack Compose, both mutableStateOf and derivedStateOf are state management tools, but they serve different purposes.
 * 1- mutableStateOf
 *      - A way to create observable state.
 *      - When its value changes, it triggers recomposition in any composable that reads it
 *      - For UI state that changes (e.g., button toggles, form inputs, navigation flags). <<<
 *
 * 2- derivedStateOf
 *      - A way to create computed state derived from other states.
 *      - For computed values based on one or more states. <<<
 */
/**
 * # 1
...
fun DerivedStateOfExample() {
    val firstname by remember {
        mutableStateOf("Abozar")
    }
    val lastname = mutableStateOf("Raghibdoust")


    // Derived state: only recomputes if firstName or lastName changes
